...
import { Video, Copy, StopCircle, PlayCircle, Settings, Users } from 'lucide-react';
...
export default function BroadcastStudio({ onBack }) {
  ...
  useEffect(() => {
    const fetchOrInitStream = async () => {
      try {
        // Try to find an existing stream for this user
        ...
        if (records.items.length > 0) {
          ...
          // Initialize a new stream container
          const newStream = await pb.collection('cplayz_streams').create({
            ...
            viewerCount: 0,
            hypeCount: 0,
            // Mocking a Stream Key for OBS
            streamKey: `live_${pb.authStore.model.id}_${Math.random().toString(36).substr(2, 9)}`,
            streamUrl: '' // Will be populated in Phase 2 with Mux playback URL
          });
    ...
    const subscribe = async () => {
      unsub = await pb.collection('cplayz_streams').subscribe('*', (e) => {
        if (e.action === 'update' && e.record.hostId === pb.authStore.model?.id) {
  ...
  const toggleLiveStatus = async () => {
    ...
    try {
      await pb.collection('cplayz_streams').update(streamData.id, {
        isLive: !streamData.isLive,
        // Reset counters when going live
        viewerCount: !streamData.isLive ? 0 : streamData.viewerCount,
        hypeCount: !streamData.isLive ? 0 : streamData.hypeCount
      });
    } catch (err) {}
  };
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
          <h2 style={{ color: '#fff', margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
            <Video size={24} color="#3b82f6" /> Broadcast Studio
          </h2>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            ...
            <div>
              <div style={{ color: '#94a3b8', fontSize: 13, marginBottom: 4 }}>Viewers</div>
          ...
          >
            {streamData.isLive ? <StopCircle size={20} /> : <PlayCircle size={20} />}
            {streamData.isLive ? 'End Stream' : 'Go Live'}
          </button>
        </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <input 
                ...
                style={{ flex: 1, background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: '10px 12px', color: '#fff' }}
              />
              <button onClick={copyKey} style={{ background: 'rgba(255,255,255,0.1)', border: 'none', color: '#fff', padding: '0 16px', borderRadius: 8, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
